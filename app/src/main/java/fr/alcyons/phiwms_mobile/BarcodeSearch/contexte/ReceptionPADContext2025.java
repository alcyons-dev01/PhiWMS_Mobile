...
public class ReceptionPADContext2025 extends MainActivity {
    ...
    private List<PH_Reliquat> liste_phreliquat;
    ...
    public PH_Reliquat_Reception_Adapte phReliquatReceptionPUIAdapte_courant;
    public List<PH_Reliquat_Reception_Adapte> list_reliquat_receptionPuiAdapte;
    public PH_Reliquat_Reception_Adapte.Lot nouveau_lot;
    public PH_Reliquat_Reception_Adapte.ZoneEtEmplacement nouvelEmplacement;
    ...
    public ReceptionPADContext2025(final Context context, final SQLiteDatabase db, Utilisateur utilisateurConnecte, List<String> ListeGTIN, int userId, int CommandeId, List<PH_Reliquat> liste_phreliquat, List<PH_Reliquat_Reception_Adapte> list_reliquat_receptionPuiAdapte, Depot_Emplacement emplacementPrecedent, Produit produitPrecedent){
        ...
        this.liste_phreliquat = liste_phreliquat;
        this.list_reliquat_receptionPuiAdapte = list_reliquat_receptionPuiAdapte;
        serialisationService = new Serialisation(context, db, utilisateurConnecte);
        nouveau_lot = null;
        validation = true;
        stringList = new ArrayList<>();
        listeSerie = new ArrayList<>();
...
/*        else if(emplacement_courant == null)
        ...
        else if(chaine.startsWith("PHITAGPLACE") && nouveau_lot == null)
        {
            ...
            if(activityName.contentEquals("ScannerReceptionActivity"))
            {
                ((ScannerReceptionActivity)context).afficherSnackBar("Veuillez scanner une référence");
            }
        ...
        else if(emplacement_courant == null && nouveau_lot != null)
        {
            if(activityName.contentEquals("ScannerReceptionActivity"))
            {
                ((ScannerReceptionActivity)context).afficherSnackBar("Veuillez scanner un emplacement");
            }
            else if(activityName.contentEquals("ScannerPreparationActivity"))
            {
        ...
        else
        {
            if(nouveau_lot == null)
            {
                ...
                {
                    //gestion de l'emplacement précédent si c'est le même produit qui est scanné
                    ...
                    for(PH_Reliquat courant : liste_phreliquat)
                    {
                        if(courant.getProduitID() == produitCourant.getID_produit())
                        {
                            produit_present = true;
                            reliquat_id_courant = courant.getReliquat_UID();
                            reliquat_courant = PH_ReliquatOpenHelper.getPH_ReliquatById(db, reliquat_id_courant);
                ...
                else if(serie != null && !serie.contentEquals("") && produitCourant.isSerialiser_Reception_Delivrance() && produitCourant.isSuivi_Serialisation() && stringList.indexOf(chaine) == -1)
                {
                    if(produit != null)
                    {
                        ...
                        codeInconnu = false;
                        codeInconnu = false;
                        PH_Reliquat ph_reliquat = PH_ReliquatOpenHelper.getPH_ReliquatById(db, reliquat_id_courant);

                        for(PH_Reliquat_Reception_Adapte courant: list_reliquat_receptionPuiAdapte)
                        {
                            if(courant.getPhReliquatUID() == ph_reliquat.getReliquat_UID())
                            {
                                phReliquatReceptionPUIAdapte_courant = courant;
                                break;
                            }
                        }
...
/*                    if(serie != null && !serie.contentEquals("") && !listeSerie.contains(serie))
                    {
                        listeSerie.add(serie);
                        PH_Reliquat ph_reliquat = PH_ReliquatOpenHelper.getPH_ReliquatById(db, reliquat_id_courant);

                        for(PH_Reliquat_ReceptionPUI_Adapte courant: list_reliquat_receptionPuiAdapte)
                        {
                            if(courant.getPhReliquatUID() == ph_reliquat.getReliquat_UID())
                            {
                                phReliquatReceptionPUIAdapte_courant = courant;
                    ...
                    else if(serie != null && !serie.contentEquals("") && listeSerie.contains(serie))
                    {
                        ((ScannerPreparationActivity) context).afficherSnackBar("Le numéro de série à déjà était scanné");
                        nouveau_lot = null;
                        qte_lot_courant = 0;
                        phReliquatReceptionPUIAdapte_courant = null;
                        validation = true;
        ...
        if (data != null) {
            switch (requestCode) {
                case CodesEchangesActivites.RETOUR_SCANNER:
                    list_reliquat_receptionPuiAdapte = new ArrayList<>();
                    ...
                    list_reliquat_receptionPuiAdapte = (List<PH_Reliquat_Reception_Adapte>) data.getExtras().getSerializable("reliquatAdapteList");
                    break;
            }
        }
    }
...
/*        else if(emplacement_courant == null)
    ...
    public void ValiderScan()
    {
        if(phReliquatReceptionPUIAdapte_courant != null)
        {
            ...
            for(PH_Reliquat_Reception_Adapte.Lot lot_courant : phReliquatReceptionPUIAdapte_courant.getlotList())
            {
                index_lot++;
                if(lot_courant.getNumeroLot().contentEquals(nouveau_lot.getNumeroLot()))
                {
                    for(PH_Reliquat_Reception_Adapte.ZoneEtEmplacement zoneEtEmplacement_courant : lot_courant.getZoneEtEmplacementList())
                    {
                        index_emplacement ++;
            ...
            //on change dans la liste le reliquat adapte liste
            int index = -1;
            for(int i = 0; i < list_reliquat_receptionPuiAdapte.size(); i++)
            {
                if(list_reliquat_receptionPuiAdapte.get(i).getPhReliquatUID() == phReliquatReceptionPUIAdapte_courant.getPhReliquatUID())
                {
                    index = i;
                    break;
                }
            ...
            list_reliquat_receptionPuiAdapte.remove(index);
            list_reliquat_receptionPuiAdapte.add(phReliquatReceptionPUIAdapte_courant);
