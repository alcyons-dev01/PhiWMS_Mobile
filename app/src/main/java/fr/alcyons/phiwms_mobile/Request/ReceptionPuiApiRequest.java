...
/**
 * Requête Volley pour charger les commandes PUI depuis l'API.
 *
 * Avant refactoring : cette logique était inline dans getObjectRequest(),
 * une méthode de 80+ lignes dans l'Activity, mêlant parsing JSON,
 * persistance BDD et callbacks UI.
 */
public class ReceptionPuiApiRequest extends JsonObjectRequest {
    ...
    public interface OnErreurCallback {
        void onErreur(String messageUtilisateur);
    }

    // Le token est stocké en champ pour être accessible dans getHeaders(),
    // ce qui évite la classe anonyme dans super() — illégale en Java.
    ...
    /** Point d'entrée public — remplace le constructeur public d'origine. */
    ...
    private static void traiterErreurApi(Context context,
                                         JSONObject response,
                                         OnErreurCallback onErreur) throws Exception {
    ...
    private static void persisterEtNotifier(SQLiteDatabase db,
                                            JSONObject response,
                                            ReceptionPuiViewModel viewModel,
                                            OnSuccessCallback onSuccess) throws Exception {
