...
public class ReassortAdapter extends BaseAdapter {

    private static final int TYPE_ITEM = 0;
    private static final int TYPE_HEADER = 1;
    public ArrayList<PH_Reassort> listeReassort = new ArrayList<PH_Reassort>();
    ...
    public ReassortAdapter(Context context, SQLiteDatabase database, Utilisateur utilisateur) {
    ...
    public void addItem(final PH_Reassort item) {
        listeReassort.add(item);
        notifyDataSetChanged();
    }

    public void addSectionHeaderItem(final PH_Reassort item) {
        listeReassort.add(item);
        sectionHeader.add(listeReassort.size() - 1);
        notifyDataSetChanged();
    }
    ...
    @Override
    public int getCount() {
        return listeReassort.size();
    }

    @Override
    public PH_Reassort getItem(int position) {
        return listeReassort.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ReassortViewHolder viewHolder = (ReassortViewHolder) convertView.getTag();

        if(viewHolder == null)
        {
            viewHolder = new ReassortViewHolder();
            switch (rowType) {
                case TYPE_ITEM:
                    convertView =  mInflater.inflate(R.layout.row_dotation, parent, false);
        ...
        if (rowType == TYPE_ITEM) {
            PH_Reassort reassortCourant = (PH_Reassort) getItem(position);
            int nbDetail = 0;
    ...
    private class ReassortViewHolder {
        public TextView nom;
