...
public class CommandeReceptionPUIAdapter extends ArrayAdapter implements Filterable {
    ...
    CommandeReceptionPUIAdapter.CommandeReceptionPUIFilter filter;

    public CommandeReceptionPUIAdapter(Context context, SQLiteDatabase db, List<Commande> commandeList) {
    ...
    public Filter getFilter() {
        if (filter == null)
            filter = new CommandeReceptionPUIAdapter.CommandeReceptionPUIFilter();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.row_commande_reception_pui, parent, false);
        }

        // Récupération des objets graphiques
        CommandeReceptionPUIAdapter.ProduitViewHolder viewHolder =(CommandeReceptionPUIAdapter.ProduitViewHolder) convertView.getTag();

        if (viewHolder == null) {
            viewHolder = new CommandeReceptionPUIAdapter.ProduitViewHolder();
        ...
        if (commandeCourant.getSituation().equals("L")) {
            viewHolder.zone_info_reception.setBackgroundColor(context.getResources().getColor(R.color.orange2));
            viewHolder.ZoneNbColisEnAttente.setBackgroundColor(context.getResources().getColor(R.color.orange2));
            viewHolder.relative_principal.setBackground(context.getResources().getDrawable(R.drawable.background_preparation_orange));

        } else {
            viewHolder.zone_info_reception.setBackgroundColor(context.getResources().getColor(R.color.bleu_clair_alcyons));
            viewHolder.ZoneNbColisEnAttente.setBackgroundColor(context.getResources().getColor(R.color.orange2));
            viewHolder.relative_principal.setBackground(context.getResources().getDrawable(R.drawable.background_preparation_bleu));
        }
        ...
        if (dateLiv != null) {
            ...
            if (delaiLivraison >= delaiExpire) {
                viewHolder.livraisonPrevueDate.setTextColor(context.getResources().getColor(R.color.rouge2));
            } else if (delaiLivraison >= delaiDemain) {
                viewHolder.livraisonPrevueDate.setTextColor(context.getResources().getColor(R.color.rouge2));
            } else if (delaiLivraison >= delaiApresDemain) {
                viewHolder.livraisonPrevueDate.setTextColor(context.getResources().getColor(R.color.orange2));
            } else {
                viewHolder.livraisonPrevueDate.setTextColor(context.getResources().getColor(R.color.vert));
            }
    ...
    private class CommandeReceptionPUIFilter extends android.widget.Filter {
        @Override
        protected FilterResults performFiltering(CharSequence constraint) {

            commandeList.clear();
            for (Commande commandeCourant : commandeDeBaseList) {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            if(results != null && results.values != null)
            {
                ...
                for (Commande commande : (List<Commande>) results.values) {
                    add(commande);
                }
