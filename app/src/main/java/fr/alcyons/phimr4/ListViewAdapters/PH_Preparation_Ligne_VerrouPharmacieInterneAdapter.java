...
import fr.alcyons.phimr4.VerrouPharmacieInterne.DetailVerrouPharmacieInterneActivity;


public class PH_Preparation_Ligne_VerrouPharmacieInterneAdapter extends ArrayAdapter {
    ...
    PH_Preparation_Ligne_VerrouPharmacieInterneAdapter.OnDataChangeListener mOnDataChangeListener;
    ...
    View.OnClickListener clicDataMatrix = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            ((DetailVerrouPharmacieInterneActivity) context).decoderCodeBarre((EditText) numLot, date, viewHolderSelectionne);
        }
    };

    public PH_Preparation_Ligne_VerrouPharmacieInterneAdapter(Context context, SQLiteDatabase database, List<PH_Preparation_Ligne_VerrouPharmacie_Adapte> phPreparationLigneVerrouPharmacieAdapteList) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (phPreparationLigneVerrouPharmacieAdapte != null) {
            ...
            viewHolder.qteParLot.setText(phPreparationLigneVerrouPharmacieAdapte.getQteParLot());
            viewHolder.statutLigne.setText(viewHolder.valeurStatut);
            viewHolder.numLot.setFocusable(false);
            viewHolder.datePeremption.setClickable(false);
            if(!viewHolder.produit_inconnu)
            {
                ...
                if (viewHolder.valeurDate.equals("")) {
                    ...
                    try {
                        if (phPreparationLigneVerrouPharmacieAdapte.getPeremptionDate().length() >= 10) {
                            if(phPreparationLigneVerrouPharmacieAdapte.getPeremptionDate().contentEquals("0000-00-00"))
                            {
                                viewHolder.valeurDate = "";
                            }
                            else
                            {
                ...
                if (!viewHolder.valeurDate.equals("") || !viewHolder.valeurDate.equals("00/00/0000")) {
            ...
            viewHolder.qteParLot.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    PH_Preparation_Ligne preparation_ligneCourant = phPreparationLigneVerrouPharmacieAdapteList.get(position).getPhPreparationLigne();
                    PH_Lot_Ligne lot_ligne = PH_Lot_LigneOpenHelper.getListePH_Lot_LigneByPreparationLigneLotSerie(db, preparation_ligneCourant, phPreparationLigneVerrouPharmacieAdapte.getNumLot(), phPreparationLigneVerrouPharmacieAdapte.getSerieNumero());
                    if(viewHolderList.get(position).valeurStatut.contentEquals("Vérouillée"))
                    {
                        ...
                        viewHolderList.get(position).layoutQuantite.setBackgroundColor(context.getColor(R.color.vert));
                        lot_ligne.setVerrouiller(false);
                        ((DetailVerrouPharmacieInterneActivity) context).majColis(lot_ligne, false);
                    }
                    else
                    {
                        ...
                        viewHolderList.get(position).layoutQuantite.setBackgroundColor(context.getColor(R.color.rouge2));
                        lot_ligne.setVerrouiller(true);
                        ((DetailVerrouPharmacieInterneActivity) context).majColis(lot_ligne, true);
                    }
                    //background_cadre_vert
                    PH_Lot_LigneOpenHelper.mettreAJourPHLotLigne(db, lot_ligne);
                }
            });
            ...
            if(phPreparationLigneVerrouPharmacieAdapte.getSerieNumero() != null || !phPreparationLigneVerrouPharmacieAdapte.getSerieNumero().contentEquals("") || !phPreparationLigneVerrouPharmacieAdapte.getSerieNumero().contentEquals("null"))
            {
                ...
                if(serialisation_courant != null)
                {
                    ...
                    if(resultat_serialisation.contentEquals("INACTIVE") || resultat_serialisation.contentEquals("UNKNOWN"))
                    {
                        if(first_passage)
                        {
                            if(viewHolder.valeurDate.contentEquals("00/00/0000") || viewHolder.valeurDate.contentEquals(""))
                            {
    ...
    public class PH_Preparation_Ligne_AdapteViewHolder {
        ...
        public PH_Preparation_Ligne_AdapteViewHolder(PH_Preparation_Ligne_VerrouPharmacie_Adapte phPrepLigne) {
            ...
            if(phPrepLigne.isVerouiller())
                this.valeurStatut = "Vérouillée";
            else
                this.valeurStatut = "Dévérouillée";
            ...
            Date date = null;
            try {
                if(phPrepLigne.getPeremptionDate().contentEquals("0000-00-00") ||phPrepLigne.getPeremptionDate().contentEquals("00/00/0000"))
                {
                    date = null;
                }
                else
