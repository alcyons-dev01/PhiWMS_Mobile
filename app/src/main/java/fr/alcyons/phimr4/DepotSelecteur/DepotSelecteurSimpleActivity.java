...
public class DepotSelecteurSimpleActivity extends ServiceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(vide)
        {
            afficherSnackBarVide(intent.getExtras().getString("service"));
        }

        String depotType = getIntent().getExtras().getString("depotType");
        ...
        if (listeReduite) {
            List<String> depotReferenceList = new ArrayList<>();
            ...
            for (String depotReference : depotReferenceList) {
                Depot depot = gestionnaireDepot.getDepotParReference(db, depotReference);
                depotList.add(depot);
            }
        } else {
            depotList = gestionnaireDepot.getDepotsParType(db, depotType);
        }

        Collections.sort(depotList, new Comparator<Depot>() {
            @Override
        ...
        boutonGeolocalisation.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (latitude == 0 && longitude == 0) {
                    ...
                    Depot depotLePlusProche = gestionnaireDepot.getDepotParID(db, depotLesPlusProche.get(0).getKey());
                    ...
                    for (DepotOpenHelper.CustomObject customObject : depotLesPlusProche) {
                        Depot depotLePlusProche = gestionnaireDepot.getDepotParID(db, customObject.getKey());
    ...
    public void afficherSnackBarVide(String service) {
        ...
        Snackbar snackbar = Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content), Html.fromHtml("<b>"+phrase+"</b>", 0), Snackbar.LENGTH_LONG);;
