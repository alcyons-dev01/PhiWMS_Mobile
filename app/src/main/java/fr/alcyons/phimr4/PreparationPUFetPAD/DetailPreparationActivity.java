...
import fr.alcyons.phimr4.ListViewAdapters.AlertePreparationAdapter;
import fr.alcyons.phimr4.ListViewAdapters.PH_Preparation_Ligne_PreparationLotAdapter;
import fr.alcyons.phimr4.Outils.Alerte;
import fr.alcyons.phimr4.Outils.Chronometer;
...
import fr.alcyons.phimr4.ReceptionPUI.DetailReceptionPuiActivity;
...
public class DetailPreparationActivity extends ServiceAvecConnexionActivity {
    ...
    public void enregistrerPhPreparation()
    {
        ...
        for (PH_Preparation_Ligne_Preparation_Adapte ph_preparationLigneAdapte : ph_preparation_ligne_preparationLotAdapter.ph_preparation_lignes_Adaptes) {
            ...
            if (nbLignesAvecValeur == 0) {
                nbLigneSansValeur ++;
                PH_Preparation_Ligne ph_preparation_ligne = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, ph_preparationLigneAdapte.getPh_preparationLigneID());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ph_preparation_Selectionne = gestionnairePH_Preparation.getPH_PreparationByID(db, intent.getExtras().getInt("ph_preparationUID_Selectionne"));
        ...
        phPreparationLignes = gestionnairePH_Preparation_Ligne.getAllPHPreparationLignesParPHPreparation(db, ph_preparation_Selectionne);
        ...
        phPreparationLigne_ListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                PH_Preparation_Ligne ph_preparationLigne = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, ph_preparationLigneAdapte.getPh_preparationLigneID());
                ...
                Depot depot = gestionnaireDepot.getDepotParReference(db, ph_preparation_Selectionne.getDepotOrigineReference());
                                    ...
                                    if (nbResultat == 0) {
                                        ...
                                        for (int k = 0; k < ph_preparationLigne_JSONArray.length(); k++) {
                                            ...
                                            for (int y = 0; y < phStockLotEmplacement_JSONArray.length(); y++) {
                                                ...
                                                Stock_Lot_Emplacement_Light stock_lot_emplacement_bdd = gestionnaireStock_Lot_Emplacement.getStock_Lot_EmplacementByID(db, stock_lot_emplacement_light.get_UID());
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        gestionnaireStock_Lot_Emplacement.mettreAJourUnStockLotEmplacement(db, stock_lot_emplacement_light);
                                                    }
                                                }
                                            }
                                        }
                ...
                for (PH_Preparation_Ligne phPrepLigne : phPreparationLignes) {
                    ...
                    if ((phPrepLigne.getQte_APreparer() > 0 || phPrepLigne.getQte_Demander() == phPrepLigne.getQte_preparer()) && phPrepLigne.getQte_APreparer() != 0) {
                        ...
                        if(produit != null)
                        {
                            ...
                            int qtePreparerTemp = phPrepLigne.getQte_preparer();
                            for (Stock_Lot_Emplacement_Light stockLotEmplacement : stock_lot_emplacement_lightList) {
                                if (stockLotEmplacement.getQte() > 0) {
                                    ...
                                    gestionnaireStock_Lot_Emplacement.mettreAJourUnStockLotEmplacement(db, stockLotEmplacement);
                ...
                case CodesEchangesActivites.RETOUR_CODE_GS1: {
                    if (resultCode == DetailPreparationActivity.RESULT_OK) {
                        ...
                        if (gs1Decoupe.size() != 0) {
                            ...
                            produit_List = gestionnaireProduit.getMedicamentsParGTIN(db, codeGtin);
                            if (produit_List.size() == 1) {
                                ...
                                for (PH_Preparation_Ligne_Preparation_Adapte phPreparationLignePreparationAdapte : phPreparationLignePreparationAdapte_List) {
                                    PH_Preparation_Ligne phPreparationLigne = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, phPreparationLignePreparationAdapte.getPh_preparationLigneID());
    ...
    private void onClickTriDesignation()
    {
        ...
        Collections.sort(phPreparationLignePreparationAdapte_List, new Comparator<PH_Preparation_Ligne_Preparation_Adapte>() {
            @Override
            public int compare(PH_Preparation_Ligne_Preparation_Adapte o1, PH_Preparation_Ligne_Preparation_Adapte o2) {

                PH_Preparation_Ligne oo1 = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, o1.getPh_preparationLigneID());
                PH_Preparation_Ligne oo2 = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, o2.getPh_preparationLigneID());
    ...
    private void onMenuDatamatrixClick() {
        ...
        if(liste_ph_preparation_ligne.size() == 0)
        {
    ...
    public void onBackPressed() {
        if(ph_preparation_ligne_preparationLotAdapter.ph_preparation_lignes_Adaptes.size() > 0)
        {
    ...
    public void EnregistrerPreparation(int nbTotalLotsAvecValeurSaisie)
    {
        ...
        for (PH_Preparation_Ligne_Preparation_Adapte ph_preparationLigneAdapte : ph_preparation_ligne_preparationLotAdapter.ph_preparation_lignes_Adaptes) {
            ...
            ph_preparationLigneCorrespondant = gestionnairePH_Preparation_Ligne.getPH_Preparation_LigneByID(db, ph_preparationLigneAdapte.getPh_preparationLigneID());
            ...
            if(ph_preparationLigneCorrespondant != null)
            {
                ...
                for (PH_Preparation_Ligne_Preparation_Adapte.LotAdapte lot : lotsSaisis) {
                    ...
                    long rowID = gestionnairePH_Preparation_Ligne.insererUnPH_Preparation_LigneEnBDD(db, ph_preparationLigneCourant);
                    if (rowID != -1) {
                        compteurReussiteParLotAvecValeur++;
                        gestionnaireElementASynchroniser.ajouterElementASynchroniser(db, PH_Preparation_LigneOpenHelper.Constantes.TABLE_PH_PREPARATION_LIGNE, ph_preparationLigneCourant.getPhiMR4UUID(), ph_preparationLigneCourant.get_UID(), DBOpenHelper.ActionsEAS.AJOUT);
                        ...
                        if(stock_lot_emplacement != null)
                        {
                            ...
                            gestionnaireElementASynchroniser.ajouterElementASynchroniser(db, Stock_Lot_EmplacementLightOpenHelper.Constantes.TABLE_STOCK_LOT_EMPLACEMENT, stock_lot_emplacement.getPhiMR4UUID(), stock_lot_emplacement.get_UID(), DBOpenHelper.ActionsEAS.MAJ);
                        }
                    }
                }
                gestionnaireElementASynchroniser.ajouterElementASynchroniser(db, PH_Preparation_LigneOpenHelper.Constantes.TABLE_PH_PREPARATION_LIGNE, ph_preparationLigneCorrespondant.getPhiMR4UUID(), ph_preparationLigneCorrespondant.get_UID(), DBOpenHelper.ActionsEAS.SUPPR);
                gestionnairePH_Preparation_Ligne.supprimerUnPhPreparationLigne(db, ph_preparationLigneCorrespondant);
        ...
        if (compteurReussiteParLotAvecValeur == nbTotalLotsAvecValeurSaisie) {
            ...
            long rowID = gestionnairePH_Preparation.mettreAJourUnPHPreparation(db, ph_preparation_Selectionne);
            if (rowID != -1) {
                gestionnaireElementASynchroniser.ajouterElementASynchroniser(db, PH_PreparationOpenHelper.Constantes.TABLE_PH_PREPARATION, ph_preparation_Selectionne.getPhiMR4UUID(), ph_preparation_Selectionne.getUID(), DBOpenHelper.ActionsEAS.MAJ);
            } else {
        ...
        listePhPreparationRALListe = gestionnairePH_Preparation_Ligne.getAllPHPreparationLignesRAL(db, ph_preparation_Selectionne);
        ...
        if (OutilsGestionConnexionReseau.isServerAccessible(DetailPreparationActivity.this)) {
            gestionnaireElementASynchroniser.toutSynchroniser(DetailPreparationActivity.this, db, utilisateurConnecte, true);
        }
