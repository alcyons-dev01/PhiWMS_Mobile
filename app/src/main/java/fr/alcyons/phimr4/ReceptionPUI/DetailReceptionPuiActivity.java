...
public class DetailReceptionPuiActivity extends ServiceActivity {
    ...
    public void envoyerMail(boolean copieMail, String email)
    {
        if (copieMail) {
    ...
    private class SendEmailTask extends AsyncTask<String, Object, Object> {
        ...
        protected Object doInBackground(String... email) {
            ...
            if(!envoyerCopie)
            {
                sender = new Mail(DetailReceptionPuiActivity.this, email[0], true, db);
            }
            else
            {
                sender = new Mail(DetailReceptionPuiActivity.this, email[0], EmailCopie, true, db);
            }
            try {
    ...
    public void onBackPressed() {
        if(phReliquatReceptionPUIAdapteList.get(0).getlotList().size() > 0)
        {
    ...
    private void retourService(final Bundle bundle)
    {
        ...
        Bundle detailReceptionPUIBundle = bundle;
        detailReceptionPUIIntent.putExtras(detailReceptionPUIBundle);
