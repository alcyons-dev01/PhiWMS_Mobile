...
/**
 * Created by olivier on 07/02/2018.
 */
...
public class InformationDotationServiceActivity extends ServiceAvecConnexionActivity {
    ...
    List<Demande_PUI> demandePuiList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dotation = gestionnaireDotation.getDotationByPhiMR4UUID(db, dotationId);

        //Initialisation des objets graphiques
        nomDemandeTextView = (TextView) findViewById(R.id.nomDemande);
        demandePuiListeView = (ListView) findViewById(R.id.listeView);

        //initialisation du boolean permettant de dire si il n'y a aucune dotation
        vide = false;

        nomDemandeTextView.setText(dotation.getIntitulé().trim());
    ...
    public void onResume() {
        ...
        if (OutilsGestionConnexionReseau.isServerAccessible(InformationDotationServiceActivity.this) && passageParOnCreate) {

            if (!swipeRefreshLayout.isRefreshing()) {
                mProgressDialog = ProgressDialog.show(InformationDotationServiceActivity.this, "Veuillez patienter", "Synchronisation des Dotations en cours");
            }

            RequestQueue requestQueue = Volley.newRequestQueue(InformationDotationServiceActivity.this);
            String urlRequete = "";
            try {
                urlRequete = ParametresServeurOpenHelper.getPartieCommuneUrls(db) + DBOpenHelper.Urls.uriRequetedetail_dots + URLEncoder.encode(depot.getDepot_Reference(), "utf-8") + "/" + dotation.get_UID();
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                ...
                                if (nbResultat == 0) {
                                    ...
                                    JSONArray detail_dotsJson = response.getJSONArray("Detail_dot");
                                    for (int k = 0; k < detail_dotsJson.length(); k++) {
                                        JSONObject detail_dotJson = detail_dotsJson.getJSONObject(k);
                                        long rowID = gestionnaireDetail_Dot.insererDetail_DotEnBDD(db, new Detail_Dot(detail_dotJson));
                                        if (rowID != -1) {
                                            JSONArray ph_stock_destinataire = detail_dotJson.getJSONArray("ph_stock_destinataire");
                                            JSONArray ph_stock_pui = detail_dotJson.getJSONArray("ph_stock_pui");
        ...
        for (Detail_Dot detail_dot : Detail_DotOpenHelper.getAllDetailDotParDotation(db, dotation)) {
            ...
            Depot depotDestinataire = DepotOpenHelper.getDepotParReference(db, dotation.getRef_Depot());
        ...
        //tri de la liste
        Collections.sort(demandePuiList, new Comparator<Demande_PUI>() {
            @Override
            public int compare(Demande_PUI o1, Demande_PUI o2) {
                return o1.getDésignation().compareTo(o2.getDésignation());
            }
        });

        // Initialisation de l'adapter puis transfere de l'adapter à la listeView pour l'affichage
        demandePuiAdapter = new Demande_PUIAdapter(InformationDotationServiceActivity.this, demandePuiList);
        demandePuiListeView.setAdapter(demandePuiAdapter);
    ...
    public void onClick_Action_informations() {
        ...
        Alerte.afficherAlerteInfoDepot(InformationDotationServiceActivity.this, InformationDotationServiceActivity.this.getLayoutInflater(), depotNom, depotAdresse, depotTelephone, depotFax);
    }
    ...
    public void onClick_Action_prochaineLivraison() {
        ...
        datePrevisionDuTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog datePrevisionDuPickerDialog = new DatePickerDialog(InformationDotationServiceActivity.this, datePrevisionDuDatePicker, year, month, day);
        ...
        datePrevisionAuTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog datePrevisionAuPickerDialog = new DatePickerDialog(InformationDotationServiceActivity.this, datePrevisionAuDatePicker, year, month, day);
                ...
                if (!dateMinString.contentEquals("00/00/0000")) {
                    ...
                    datePrevisionAuPickerDialog = new DatePickerDialog(InformationDotationServiceActivity.this, datePrevisionAuDatePicker, year, month, day);
    ...
    public void onClick_Action_envoyerDemande() {
        ...
        if (confirmer) {
            ...
            String Origine = "";
            String Liste = "Dotation Globale : " + dotation.getIntitulé();
            ...
            ActionUtilisateur new_action_utilisateur = new ActionUtilisateur(actionId, utilisateurConnecte.getId(), date_string, serviceActuel.getId(), Integer.parseInt(ParametresServeurOpenHelper.getPortServeur(db)), "En attente", ph_preparation.getUID(), "", "Dotation Service");
            ...
            for (Demande_PUI demande_pui : demandePuiAdapter.demandePuiList
                ...
                Montant_TTC = 0;
                double PoidsTotal = 0;
                String depot_Destinataire_Reference = dotation.getRef_Depot();
            ...
            if (compteurReussite == demandePuiAdapter.demandePuiList.size()) {
                ...
                if (OutilsGestionConnexionReseau.isServerAccessible(InformationDotationServiceActivity.this)) {
                    gestionnaireElementASynchroniser.toutSynchroniser(InformationDotationServiceActivity.this, db, utilisateurConnecte, true);
                }

                Toast.makeText(InformationDotationServiceActivity.this, "Demande Dotation Service effectuée", Toast.LENGTH_SHORT).show();
                InformationDotationServiceActivity.this.finish();
                return;
            } else {
                Alerte.afficherAlerte(InformationDotationServiceActivity.this, "Alerte", "Une erreur est survenue", "alerte");
