...
import static com.example.phiwms_mobile.BaseDeDonnees.ServiceOpenHelper.Constantes.NUM_COL_ID_PERIMETRE_FONCTIONNEL_SERVICE;
import static com.example.phiwms_mobile.BaseDeDonnees.ServiceOpenHelper.Constantes.NUM_COL_NOM_PERIMETRE_FONCTIONNEL_SERVICE;
import static com.example.phiwms_mobile.BaseDeDonnees.ServiceOpenHelper.getServicesSeparesParPerimetreFonctionnel;
...
public class PerimetreFonctionnelOpenHelper extends DBOpenHelper {
    ...
    public static void insererPerimetresFonctionnels(SQLiteDatabase db) {
        ...
        Cursor cursor = getServicesSeparesParPerimetreFonctionnel(db);

        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                // Récupération des attributs du périmètre fonctionnel courant
                int perimetreFonctionnelID = cursor.getInt(NUM_COL_ID_PERIMETRE_FONCTIONNEL_SERVICE);
                String perimetreFonctionnelNom = cursor.getString(NUM_COL_NOM_PERIMETRE_FONCTIONNEL_SERVICE);
