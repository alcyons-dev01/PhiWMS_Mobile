 ...
 */
package com.example.phiwms_mobile.BarcodeSearch.camera;
...
import com.example.phiwms_mobile.BarcodeSearch.BarcodeGraphic;
...
public class CameraSource {
    ...
    public static class Builder {
         ...
         */
        public CameraSource build(GraphicOverlay<BarcodeGraphic> graphicOverlay) {
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            byte[] byteArray = data;
            if (camera.getParameters().getColorEffect().toString().equals(Camera.Parameters.EFFECT_NEGATIVE)) {
                /*Camera.Parameters params = camera.getParameters();
                params.setColorEffect(Camera.Parameters.EFFECT_NONE);
                camera.setParameters(params);
                for (int i = 0; i < data.length ; i++)
                {
                    byteArray[i] = (byte) (data[i] + 150);
                }*/
                /*Bitmap bitmap = BitmapFactory.decodeByteArray(byteArray, 0, data.length - 1);
                Bitmap bitmapInverse = bitmap.copy(Bitmap.Config.ARGB_8888, true);


                // Get info about Bitmap
                int width = bitmapInverse.getWidth();
                int height = bitmapInverse.getHeight();
                int pixels = width * height;

                // Get original pixels
                int[] pixel = new int[pixels];
                bitmapInverse.getPixels(pixel, 0, width, 0, 0, width, height);

                // Modify pixels
                for (int i = 0; i < pixels; i++)
                    pixel[i] ^= 0x00FFFFFF;
                bitmapInverse.setPixels(pixel, 0, width, 0, 0, width, height);

                ByteArrayOutputStream stream = new ByteArrayOutputStream();
                bitmapInverse.compress(Bitmap.CompressFormat.PNG, 100, stream);
