 ...
 */
package com.example.phiwms_mobile.BarcodeSearch.negative.camera;
...
import com.example.phiwms_mobile.BarcodeSearch.negative.filter.CameraFilter;
import com.example.phiwms_mobile.BarcodeSearch.negative.filter.NegativeFilter;
import com.example.phiwms_mobile.BarcodeSearch.negative.filter.OriginalFilter;
import com.example.phiwms_mobile.BarcodeSearch.negative.graphic.BarcodeGraphic;
import com.example.phiwms_mobile.BarcodeSearch.negative.opengl.MyGLUtils;
...
public class CameraSource {
    ...
    public static CameraSourcePreview mCameraSourcePreview;
    public static boolean negativeEffect;
    ...
    public void enableCameraSound(boolean sound) {
    ...
    public CameraSource start() throws IOException {
        synchronized (mCameraLock) {
            ...
            ArrayList<Camera.Area> listeZonesAutofocus = new ArrayList<>();
            // listeZonesAutofocus.add(new Camera.Area(mGraphicOverlay.getAutofocusRect(), 1));
            // parameters.setFocusAreas(listeZonesAutofocus);
            parameters.setVideoStabilization(true);
            mCamera.setParameters(parameters);

            cameraTextureId = MyGLUtils.genTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES);
            mDummySurfaceTexture = new SurfaceTexture(cameraTextureId);
            mDummySurfaceTexture.setDefaultBufferSize(mCameraSourcePreview.gwidth, mCameraSourcePreview.gheight);
    ...
    public void setNegativeEffect(boolean negatif) {
        if (mCamera != null) {
            if (negatif) {
                //Original
                cameraFilter = new OriginalFilter(mContext);
                negativeEffect = false;
            } else {
                //Negative
                cameraFilter = new NegativeFilter(mContext);
                negativeEffect = true;
            }
     ...
     * (between {@link #start()} or {@link #start()} and before {@link #stop()} or {@link #release()}).
     * <p/>
    ...
    public static class Builder {
        ...
        /**
         * Sets the requested frame rate in frames per second.  If the exact requested value is
         ...
         */
        public CameraSource build(GraphicOverlay<BarcodeGraphic> graphicOverlay, CameraSourcePreview cameraSourcePreview, Boolean useNegative) {
            mGraphicOverlay = graphicOverlay;
            mCameraSourcePreview = cameraSourcePreview;
            negativeEffect = useNegative;
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            byte[] byteArray = data;
            mFrameProcessor.setNextFrame(byteArray, camera);
        }
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        public void run() {
            ...
            initGL(mCameraSourcePreview.mSurfaceTexture);

            setNegativeEffect(negativeEffect);

            while (true) {
                if (mCameraSourcePreview.gwidth < 0 && mCameraSourcePreview.gheight < 0)
                    GLES20.glViewport(0, 0, mCameraSourcePreview.gwidth = -mCameraSourcePreview.gwidth, mCameraSourcePreview.gheight = -mCameraSourcePreview.gheight);
                ...
                synchronized (this) {
                    if (mDummySurfaceTexture != null) {
                        mDummySurfaceTexture.updateTexImage();
                    }
                }

                // Draw camera preview
                if (cameraFilter != null)
                    cameraFilter.draw(cameraTextureId, mCameraSourcePreview.gwidth, mCameraSourcePreview.gheight);
               ...
               /* outputFrame = new Frame.Builder()
                    ...
                    outputFrame = new Frame.Builder()
                            .setBitmap(mCameraSourcePreview.mTextureView.getBitmap())
                            .setRotation(mRotation)
                            .build();
